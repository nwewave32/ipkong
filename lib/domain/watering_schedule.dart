...
class WateringSchedule {
  ...
  // TODO(v1.1): 오래 답이 없던 식물의 학습을 어떻게 다룰지.
  //
  //  factor 는 답변에서 배운 값이고, 답변은 **과거의 증거**다. 90일이면
  //  계절이 한 번 이상 바뀌고 난방·이사·화분 교체가 끼어들 수 있다. 그런데
  //  극단까지 간 factor(최대 1.6, 최소 0.6)는 그대로 계속 쓰인다.
  //
  //  이전에 `decayStaleLearning` 이라는 이름으로 `factor * 0.5 + 0.5` 가
  //  정의만 돼 있었다 (호출부 없음, b26a065 이전). **그대로 쓰면 안 된다** —
  //  멱등하지 않아서 앱을 열 때마다 부르면 1.6 → 1.3 → 1.15 → … 로 며칠
  //  만에 학습이 통째로 뭉개진다. 제대로 하려면 "이미 감쇠시켰다"를 기억할
  //  새 컬럼과 DB 마이그레이션이 필요하다.
  //
  //  값을 몰래 깎는 대신 **다시 물어보는 쪽**이 낫다. 오래 쉰 식물에
  //  [resetSettlingForSeasonChange] 를 태우면 알림이 2택에서 3택(흙 상태)으로
  //  돌아가고 그 답이 곧바로 학습에 들어간다. 멱등하고(이미 풀린 식물을 또
  //  풀어도 그대로), 새 컬럼도 필요 없다 ([Plant.lastRespondedAt] 로 판정).
  //  마음대로 정하지 않고 확인한다는 앱의 원칙과도 맞는다.
  //
  //  v1.0 에 넣지 않은 이유: factor 범위가 [0.6, 1.6] 로 좁아 최악이어도
  //  회복 가능하고, 돌아와서 두세 번 답하면 스스로 교정된다.
